...
async function chatReply({model,message,history,specialists}){
  ...
  const out=await callRootsys({model,temperature:.45,max_tokens:1400,messages:[
    {role:'system',content:`You are Chief AI for WAHH AIR chatting directly with the owner on Telegram. Speak naturally in Malay, concise and useful. You can discuss ideas and remember the recent conversation. Do not pretend a task, web research, publication, payment, or database change happened unless it actually happened. When action is needed, explain briefly and wait for owner instruction. Active temporary specialists:\n${specs}`},
    ...hist,
    {role:'user',content:message}
  ]});
...
async function webResearch(query){
  ...
  if(!key) throw new Error('Live Research belum aktif. Tambah TAVILY_API_KEY di Vercel Environment Variables.');
  ...
  if(!r.ok)throw new Error(data?.detail||data?.message||`Research provider HTTP ${r.status}`);
  return data;
}
async function researchReply({model,query,message}){
  const research=await webResearch(query||message);
  ...
  return {answer:clean(out.content,3200),sources};
}
...
async function handleConversation(supabase,chatId,message){
  ...
  if(decision.mode==='task'){
    await addMessage(supabase,conversation.id,'assistant','Arahan ditukar kepada AI Office task.','task',{decision});
    return sendNaturalCommandResult(supabase,chatId,message);
  }
  if(decision.mode==='research'){
    await sendMessage(chatId,'🔎 <b>RESEARCH AI</b>\n\nSedang buat live web research...');
    const r=await researchReply({model,query:decision.research_query,message});
    const text=`🔎 <b>WAHH AIR RESEARCH</b>\n\n${esc(r.answer)}${sourceBlock(r.sources)}`;
    await addMessage(supabase,conversation.id,'assistant',r.answer,'research',{query:decision.research_query,sources:r.sources.map(s=>({title:s.title,url:s.url}))});
