...
async function callRootsys({model,messages,temperature=.3,max_tokens=1800}){
  ...
  try{
    const r=await fetch(`${ROOTSYS_BASE_URL}/chat/completions`,{
      method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${process.env.ROOTSYS_API_KEY}`},signal:controller.signal,
    ...
    if(e?.name==='AbortError') throw new Error('Chief AI provider timeout selepas 25 saat. Cuba hantar mesej semula.');
    throw e;
