...
module.exports = async function handler(req, res) {
  ...
  if (!url || !publishable || !secret) {
    return res.status(500).json({ error: 'Server environment belum lengkap' });
  }
  ...
  if (location) {
    ...
    if (balances?.length) {
      return res.status(409).json({ error: 'Akaun masih mempunyai stok. Return/adjust stok dahulu sebelum delete.' });
    }
  }

  const { data: unsettled } = await adminClient
    .from('sales')
    .select('id')
  ...
  if (unsettled?.length) {
    return res.status(409).json({ error: 'Masih ada komisen belum Paid. Selesaikan komisen dahulu sebelum delete.' });
  }
