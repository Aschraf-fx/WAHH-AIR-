(function(){
  ...
  function ensureModal(){
    ...
    modal.innerHTML=`<div class="modal-card" style="max-width:720px;width:min(94vw,720px);max-height:92vh;overflow:auto">
      ...
      <form id="tiktokTestPostForm" class="form-stack">
        ...
        <label>Privacy
          <select id="tiktokPrivacy" required disabled><option value="">Pilih privacy...</option></select>
        ...
        <div style="padding:12px;border:1px solid var(--border,#ddd);border-radius:12px">
          ...
          <div class="muted" style="margin:4px 0 10px">Nyatakan jika video ini mempromosikan produk, servis atau brand.</div>
          <label style="display:flex;align-items:center;gap:8px;margin:6px 0"><input id="tiktokCommercial" type="checkbox"> Kandungan ini mempromosikan brand / produk</label>
          <div id="tiktokBrandOptions" class="hidden" style="margin:8px 0 0 24px">
            <label style="display:flex;align-items:center;gap:8px;margin:6px 0"><input id="tiktokBrandOrganic" type="checkbox"> Your Brand — mempromosikan WAHH AIR / bisnes sendiri</label>
            <label style="display:flex;align-items:center;gap:8px;margin:6px 0"><input id="tiktokBrandContent" type="checkbox"> Branded Content — partnership / brand pihak ketiga</label>
          </div>
        </div>
        <label style="display:flex;align-items:flex-start;gap:8px"><input id="tiktokMusicConfirm" type="checkbox" required style="margin-top:3px"> <span>By posting, I agree to TikTok's Music Usage Confirmation.</span></label>
        <label style="display:flex;align-items:flex-start;gap:8px"><input id="tiktokConsent" type="checkbox" required style="margin-top:3px"> <span>Saya telah semak video, caption, privacy, interaction settings dan commercial disclosure ini dan saya bersetuju menghantar kandungan ini ke TikTok.</span></label>
    ...
    modal.querySelector('#tiktokCommercial').addEventListener('change',()=>syncCommercialControls(modal));
    return modal;
  }

  function closeModal(modal){modal.classList.add('hidden');if(previewUrl){URL.revokeObjectURL(previewUrl);previewUrl=null;}}
  ...
  function renderVideoPreview(modal){
    ...
    previewUrl=URL.createObjectURL(file);video.src=previewUrl;meta.textContent=`${file.name} • ${(file.size/1024/1024).toFixed(2)} MB`;wrap.classList.remove('hidden');
  }

  function privacyLabel(v){return ({PUBLIC_TO_EVERYONE:'Public to everyone',MUTUAL_FOLLOW_FRIENDS:'Friends / mutual followers',FOLLOWER_OF_CREATOR:'Followers of creator',SELF_ONLY:'Only me / private'})[v]||v;}
  ...
  function syncInteractionControls(modal){
    ...
    comment.disabled=c.comment_disabled===true;if(comment.disabled)comment.checked=false;
    const privatePost=privacy==='SELF_ONLY';
    duet.disabled=c.duet_disabled===true||privatePost;stitch.disabled=c.stitch_disabled===true||privatePost;
    if(duet.disabled)duet.checked=false;if(stitch.disabled)stitch.checked=false;
  }

  function syncCommercialControls(modal){
    const on=modal.querySelector('#tiktokCommercial').checked;
    const box=modal.querySelector('#tiktokBrandOptions');
    const own=modal.querySelector('#tiktokBrandOrganic');
    const third=modal.querySelector('#tiktokBrandContent');
    box.classList.toggle('hidden',!on);
    own.disabled=!on;third.disabled=!on;
    if(!on){own.checked=false;third.checked=false;}
  }
  ...
  async function openTestPost(){
    ...
    currentCreator=null;progress.textContent='';form.reset();privacySelect.innerHTML='<option value="">Pilih privacy...</option>';privacySelect.disabled=true;
    modal.querySelector('#tiktokVideoPreviewWrap').classList.add('hidden');syncCommercialControls(modal);
    if(previewUrl){URL.revokeObjectURL(previewUrl);previewUrl=null;}
    modal.classList.remove('hidden');box.textContent='Loading creator info...';
    try{
      const r=await api({action:'creator_info'});const c=r.creator||{};currentCreator=c;
      ...
      privacy.forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=privacyLabel(v);privacySelect.appendChild(o);});
    ...
    }catch(e){box.innerHTML=`<span class="profit-negative">${escHtml(e.message)}</span>`;toast(e.message,'error');}
  }
  ...
  async function submitTestPost(e){
    ...
    const commercial=modal.querySelector('#tiktokCommercial').checked;
    const brandOrganic=modal.querySelector('#tiktokBrandOrganic').checked;
    const brandContent=modal.querySelector('#tiktokBrandContent').checked;
    const musicConfirmed=modal.querySelector('#tiktokMusicConfirm').checked;
    ...
    if(commercial&&!brandOrganic&&!brandContent)return toast('Pilih Your Brand atau Branded Content.','error');
    if(!musicConfirmed)return toast("Sahkan TikTok Music Usage Confirmation dulu.",'error');
    ...
    const commercialText=!commercial?'No commercial content':`${brandOrganic?'Your Brand':''}${brandOrganic&&brandContent?' + ':''}${brandContent?'Branded Content':''}`;
    ...
    try{
      ...
      const init=await api({action:'init_test_post',caption,file_size:file.size,file_type:file.type,privacy_level:privacy,allow_comment:allowComment,allow_duet:allowDuet,allow_stitch:allowStitch,commercial_content:commercial,brand_organic:brandOrganic,brand_content:brandContent,music_usage_confirmed:musicConfirmed});
      progress.textContent='2/3 • Upload video terus ke TikTok...';
      const upload=await fetch(init.upload_url,{method:'PUT',headers:{'Content-Type':file.type,'Content-Range':`bytes 0-${file.size-1}/${file.size}`},body:file});
      ...
      for(let i=0;i<20;i++){
        ...
        finalStatus=s.status||{};const st=String(finalStatus.status||'UNKNOWN');
      ...
      if(finalStatus?.status==='PUBLISH_COMPLETE'){toast('TikTok test post berjaya.','success');progress.innerHTML='<strong>✅ PUBLISH_COMPLETE</strong> • Video berjaya dihantar ke TikTok.';}
      else if(finalStatus?.status==='FAILED'){throw new Error(`TikTok publish gagal: ${finalStatus.fail_reason||'unknown reason'}`);}
      else{toast('Video dah dihantar. TikTok masih processing; check semula kemudian.','success');progress.textContent=`Video dah dihantar. Status terakhir: ${finalStatus?.status||'PROCESSING'}.`;}
    }catch(err){progress.innerHTML=`<span class="profit-negative">${escHtml(err.message)}</span>`;toast(err.message,'error');}
